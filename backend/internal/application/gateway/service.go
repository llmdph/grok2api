...
attemptLoop:
	...
	handleResponse:
		...
		if isTerminalRequestForbidden(credential.Provider, lastFailure) {
			...
			// Console 429 with remaining quota still gets a soft cooldown.
			if lease.QuotaMode != "" && response.StatusCode == http.StatusTooManyRequests {
				if lastFailure.QuotaExhausted || lastFailure.FreeQuotaExhausted || lastFailure.ModelQuotaExhausted {
					...
					failureHandled = true
				} else if credential.Provider == accountdomain.ProviderConsole {
					s.applyRateLimitReconciliation(ctx, credential, response.StatusCode, retryAfter, accountapp.RateLimitReconcileAvailable, nil)
					failureHandled = true
				}
