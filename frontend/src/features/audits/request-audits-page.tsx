...
function ModelRouteValue({ model, upstreamModel, reasoningEffort, account, clientKey }: { model: string; upstreamModel: string; reasoningEffort: string; account: string; clientKey: string }) {
  const { t } = useTranslation();
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <button type="button" className="block w-full min-w-0 cursor-help text-left" aria-label={t("audits.routeDetails")}>
          ...
          {reasoningEffort ? (
            <span className="mt-0.5 block truncate text-[10px] text-muted-foreground" title={effortLabel}>
              {t("audits.reasoningEffort")}: {effortLabel}
            </span>
          ) : null}
        </button>
      </TooltipTrigger>
      <TooltipContent className="w-72 max-w-[calc(100vw-2rem)] space-y-1.5 py-2" side="top" align="start">
        ...
        <div className="grid grid-cols-[auto_1fr] gap-x-3">
          <span className="text-primary-foreground/65">{t("audits.reasoningEffort")}</span>
          <span className="truncate text-right" title={effortLabel}>{effortLabel}</span>
        </div>
        <div className="grid grid-cols-[auto_1fr] gap-x-3">
