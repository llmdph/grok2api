...
export const settingsSchema = z.object({
  ...
  routing: z.object({
    ...
    videoMaxAttempts: z.union([z.literal(UNLIMITED_ROUTING_ATTEMPTS), z.literal(0), positiveInteger.max(65535)]),
...
export function toSettingsForm(config: SettingsConfigDTO): SettingsForm {
  return {
    ...
    routing: {
      ...
      cooldownMax: parseDuration(config.routing.cooldownMax), capacityWait: parseDuration(config.routing.capacityWait), maxAttempts: config.routing.maxAttempts, videoMaxAttempts: config.routing.videoMaxAttempts ?? 0,
...
export function toSettingsDTO(config: SettingsForm): SettingsConfigDTO {
  return {
    ...
    routing: {
      ...
      cooldownMax: formatDuration(config.routing.cooldownMax), capacityWait: formatDuration(config.routing.capacityWait), maxAttempts: config.routing.maxAttempts, videoMaxAttempts: config.routing.videoMaxAttempts ?? 0,
      preferFreeBuild: config.routing.preferFreeBuild,
      markBuildChatDeniedAsReauth: config.routing.markBuildChatDeniedAsReauth,
      accountIsolatedConnections: config.routing.accountIsolatedConnections,
      segmentedSelector: config.routing.segmentedSelector,
