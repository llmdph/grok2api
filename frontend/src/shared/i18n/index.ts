  ...
  "zh-CN": {
    translation: {
      ...
      keys: {
        ...
        modelAliasesConsoleSupport: "Grok Console：grok-4.6 / grok-4.5 支持 low / medium / high；grok-4.3 支持 none / low / medium / high；grok-4.20-0309-reasoning 为固定推理模式，不支持调节 effort；grok-4.20-multi-agent-0309 额外支持 xhigh。",
      ...
      docs: {
        ...
        endpointTTSVoices: "列出内置音色；自定义音色通过 Custom Voices 接口管理。",
        endpointSTT: "Console 语音转文字，支持文件上传或音频 URL，可返回词级时间戳。",
        endpointRealtimeSecrets: "创建浏览器侧 Realtime 连接使用的短期 client secret。",
        endpointRealtime: "Realtime 语音 WebSocket，由网关代持 Console 账号完成上游鉴权与双向转发。",
        endpointCustomVoices: "上传参考音频克隆自定义音色，并用于后续 TTS 与 Realtime。",
        streaming: "流式响应",
        ...
        reference: {
          ...
          fieldReferenceImages: "可选参考图片数组，用于参考图视频；每项提供 url 或 file_id，不可与 image 同时使用。",
          fieldReferenceAudios: "可选参考音频数组，最多 3 个内置 voice_id；可单独使用或与 reference_images 组合。",
          ...
          noteImageModels: "grok-imagine-image-2.0 与 grok-imagine-image-quality-2.0 使用统一图片接口，同时支持生成与编辑；模型决定上游链路与计费。",
          ...
          noteVideoReference: "参考图视频走 /videos/generations + reference_images；不可与 image 同用，resolution 最高 720p；reference_audios 使用内置 voice_id。",
          noteVideoStrict: "视频接口严格使用当前官方字段，不保留旧项目的兼容别名。",
          noteVideoEditModel: "视频编辑与延长仅支持 grok-imagine-video。",
          ...
          fieldRealtimeExpires: "client secret 有效期，使用 expires_after.seconds，最长 3600 秒。",
          fieldRealtimeSession: "绑定到 secret 的初始 session 配置，连接建立后生效。",
          fieldWSUpgrade: "必须携带标准 WebSocket Upgrade 头；普通 HTTP GET 会被拒绝。",
          fieldCustomVoiceName: "自定义音色名称。",
          fieldCustomVoiceFile: "参考音频文件，multipart 字段名 file。",
          fieldCustomVoiceGender: "可选性别标签。",
          fieldCustomVoiceTone: "可选音色风格标签。",
          fieldCustomVoiceUseCase: "可选使用场景标签。",
          ...
          noteAudioTranscriptionsCompat: "兼容 OpenAI /v1/audio/transcriptions 与 /v1/audio/translations，底层复用 /v1/stt。",
          noteTTSStream: "同一路径支持 WebSocket 流式合成：对 /v1/tts 发起 Upgrade 即可双向推送文本并接收音频分片。",
          ...
          noteCustomVoices: "自定义音色通过 /v1/custom-voices 管理，生成的 voice_id 可直接用于 TTS 与 Realtime。",
          noteCustomVoiceManage: "另支持 GET/PATCH/DELETE /v1/custom-voices/{voice_id} 与 GET /v1/custom-voices/{voice_id}/audio。",
          ...
          noteRealtimeSecret: "client secret 供浏览器短时连接使用，避免在前端暴露长期凭据。",
          ...
          noteRealtimeAuth: "调用方仍使用 grok2api 客户端 API Key；上游 Console 凭据由网关选择并代持。",
        },
      },
      errors: {
        generic: "操作失败，请稍后重试。",
    ...
    translation: {
      ...
      creativeConsole: {
        ...
        errors: {
          ...
          noSourceVideo: "Enter a source video URL first",
      ...
      docs: {
        ...
        endpointTTSVoices: "List built-in voices. Manage cloned voices through the Custom Voices API.",
        endpointSTT: "Console speech-to-text for uploaded files or audio URLs, with optional word timestamps.",
        endpointRealtimeSecrets: "Create a short-lived client secret for browser-side Realtime connections.",
        endpointRealtime: "Realtime voice WebSocket proxied by the gateway with Console account authentication.",
        endpointCustomVoices: "Clone a custom voice from reference audio for later TTS and Realtime use.",
        streaming: "Streaming",
        ...
        reference: {
          ...
          fieldReferenceImages: "Optional reference image array for reference-to-video. Each item uses url or file_id and cannot be combined with image.",
          fieldReferenceAudios: "Optional reference audio array of up to 3 built-in voice_id values. Can be used alone or with reference_images.",
          ...
          noteImageModels: "grok-imagine-image-2.0 and grok-imagine-image-quality-2.0 share this endpoint for generation and edits; the model selects the upstream path and pricing.",
          ...
          noteVideoReference: "Reference-to-video uses /videos/generations with reference_images. Do not combine with image; resolution tops out at 720p. reference_audios accepts built-in voice_id values.",
          noteVideoStrict: "Video endpoints use the current official fields without non-standard aliases.",
          noteVideoEditModel: "Video edits and extensions only support grok-imagine-video.",
          ...
          fieldRealtimeExpires: "Client-secret lifetime as expires_after.seconds, up to 3600 seconds.",
          fieldRealtimeSession: "Optional initial session config bound to the secret and applied on connect.",
          fieldWSUpgrade: "Requires a standard WebSocket Upgrade header. Plain HTTP GET is rejected.",
          fieldCustomVoiceName: "Display name for the cloned voice.",
          fieldCustomVoiceFile: "Reference audio file in the multipart field file.",
          fieldCustomVoiceGender: "Optional gender label.",
          fieldCustomVoiceTone: "Optional tone or style label.",
          fieldCustomVoiceUseCase: "Optional use-case label.",
          ...
          noteAudioTranscriptionsCompat: "Compatible with OpenAI /v1/audio/transcriptions and /v1/audio/translations, backed by /v1/stt.",
          noteTTSStream: "The same path supports streaming synthesis: upgrade /v1/tts to a WebSocket to send text and receive audio chunks.",
          ...
          noteCustomVoices: "Custom voices are managed under /v1/custom-voices, and the resulting voice_id works with TTS and Realtime.",
          noteCustomVoiceManage: "Also supports GET/PATCH/DELETE /v1/custom-voices/{voice_id} and GET /v1/custom-voices/{voice_id}/audio.",
          ...
          noteRealtimeSecret: "Client secrets are short-lived browser credentials so long-lived secrets stay off the frontend.",
          ...
          noteRealtimeAuth: "Callers still authenticate with a grok2api client API key; the gateway selects and holds Console credentials.",
        },
      },
